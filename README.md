...
Below is the diffraction pattern of B-DNA Modeled using N = 60 Points, split among two posphate backbones as well as some inner structure meant to represent the nucleobases bridging the helicies. An important feature to note is the asymmetry presented by splitting the pattern down the line X = 0. This asymmetry is a direct effect of the 3-Dimmensional nature of the DNA structure, and a representation on how one of the helicies must "cross over" the other, leading to an asymetric diffraction pattern. Representing the helix as a 2D projection of sine waves and propogating the wave via FFT looses some of this information. Second, It is important to realize the radius of the feint circle encompasing the pattern is a **direct** measurement of the modeled DNA height as can be calculated via conventional optics formuae.

Franklin's experiment did not measure one single DNA twist, she instead was beaming fibres of DNA hung vertically, and bunched together in spools of micron-sized scale, while sampling using high energy Xrays. The result of her experiment then more accurately reflects, again, the congregate diffraction of many strands of DNA. To simulate this, we can simulate diffraction over many units of DNA, however this leads to time being the limiting factor. We unfortunately cannot employ the same filtering techniques as with simpler crystals, as the diffraction structure is more dyanmic in shape. However, by recognizing Franklins experiment is agnostic to DNA's rotation around the axis from which it is repeating, we can instead use a spacial average to help aleviate some of crsytal-edge effects. By running the simulation over 36 equally spaced rotations of the crystal along the major axis of the DNA and averaging the results (and filtering out noise caused from the simulation), we can obtain a diffraction image presented below
